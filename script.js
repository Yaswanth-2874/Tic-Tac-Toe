...
function CheckWin() {
  ...
  wins.forEach((e) => {
    if (
      ...
      text[e[0]].innerText == text[e[2]].innerText &&
      text[e[0]].innerText != ""
    ) {
      document.getElementsByClassName("info")[0].innerText =
        text[e[0]].innerText + " is the winner";
      if (text[e[0]].innerText == "X" && !gameover) xscore++;
      else if (text[e[0]].innerText == "O" && !gameover) yscore++;
      gameover = true;
...
function displayScore() {
  document.getElementsByClassName(
    "scores"
  )[0].innerHTML = `X score : ${xscore}<br> Y score : ${yscore}`;
}
...
Array.from(boxes).forEach((element) => {
  ...
  element.addEventListener("click", () => {
    ...
    var cellIndex = Array.from(boxes).indexOf(clickedCell);
    if (boxtext.innerText == "" && !gameover) {
      boxtext.innerText = turn;
      document.getElementsByClassName("view")[count].innerText = `Move ${
        count + 1
      }: Player ${player} placed ${turn} on postion ${cellIndex + 1}.`;
    ...
    reset.addEventListener("click", () => {
      ...
      for (let pos = 0; pos < 9; pos++)
        document.getElementsByClassName("view")[pos].innerText = "";
    });
